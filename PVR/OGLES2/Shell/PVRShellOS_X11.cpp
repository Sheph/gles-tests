...
bool PVRShellInit::OsInitOS()
{
    ...
    */
    ...
    {
		if(m_pShell->m_pShellData->nShellDimX < 0)
		    m_pShell->m_pShellData->nShellDimX = (display_width > display_height) ? 640 : 480;

		if(m_pShell->m_pShellData->nShellDimY < 0)
		    m_pShell->m_pShellData->nShellDimY = (display_width > display_height) ? 480 : 360;
	...
	for( int i = 0; i < numMessages; i++ )
	{
		...
		switch( event.type )
		{
			...
			case ButtonRelease:
			{
				...
				{
					case 1 : m_u32ButtonState &= ~1; break;
...
bool PVRShellInit::OsGet(const prefNamePtrEnum prefName, void **pp)
{
	...
	case prefPointerLocation:
		if( m_u32ButtonState != 0 )
		{
			/* get info about current pointer position */
			XEvent event;
			XQueryPointer(m_X11Display, m_X11Window,
			...
			m_vec2PointerLocation[ 0 ] = (float)event.xbutton.x / (float)m_pShell->m_pShellData->nShellDimX;
			m_vec2PointerLocation[ 1 ] = (float)event.xbutton.y / (float)m_pShell->m_pShellData->nShellDimY;
...
int PVRShellInitOS::OpenX11Window(const PVRShell &shell)
{
    ...
    if(shell.m_pShellData->bFullScreen)
    {
		...
		WinAttibutes.event_mask = StructureNotifyMask | ExposureMask | ButtonPressMask | ButtonReleaseMask | KeyPressMask | KeyReleaseMask;
