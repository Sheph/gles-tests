...
int PVRShell::PVRShellScreenSave(
	...
	const unsigned long	nPixelReplicate,
	char				* const ofname)
{
	char *pszFileName;
	...
	*/
	...
	const int error = PVRShellWriteBMPFile(pszFileName, Width, Height, pLines, nPixelReplicate);
	FREE(pszFileName);
	if (error)
	{
		return 10*error+1;
...
#define ByteSwap(x) PVRShellByteSwap((unsigned char*) &x, sizeof(x))
	...
	if (fpDumpfile != 0)
	{
		...
		int i32OutBytesPerLine = uWidth * 3 * nPixelReplicate;
		...
		if(i32OutBytesPerLine & 3)
		{
			...
			++i32OutBytesPerLine;
			i32OutAlign = i32OutBytesPerLine - uWidth * 3 * nPixelReplicate;
		}

		unsigned char *pData = (unsigned char*) pImageData;

		{
			int i32RealSize = i32OutBytesPerLine *uHeight * nPixelReplicate;

			// BMP Header
			unsigned short  bfType = 0x4D42;
			unsigned long   bfSize = i32BMPHeaderSize + i32BMPInfoSize + i32RealSize;
			unsigned short  bfReserved1 = 0;
			unsigned short  bfReserved2 = 0;
			unsigned long   bfOffBits = i32BMPHeaderSize + i32BMPInfoSize;

			// BMP Info Header
			unsigned long  biSize = i32BMPInfoSize;
			unsigned long  biWidth = uWidth * nPixelReplicate;
			unsigned long  biHeight = uHeight * nPixelReplicate;
		...
		const unsigned char align = 0x00;

		// Write image.
		for(unsigned int nY = 0; nY < uHeight; nY++)
		{
			const unsigned char * pRow = &pData[3 * uWidth * nY];
			for(unsigned int nRepY = 0; nRepY < nPixelReplicate; ++nRepY)
			{
				for(unsigned int nX = 0; nX < uWidth; ++nX)
				{
					const unsigned char * pPixel = &pRow[3 * nX];
					for(unsigned int nRepX = 0; nRepX < nPixelReplicate; ++nRepX)
					{
						fwrite(pPixel, 1, 3, fpDumpfile);
					}
				}

				for(unsigned int i = 0; i < i32OutAlign; ++i)
